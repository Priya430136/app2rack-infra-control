...
[
  ...
  [3, "srv-node-31", 80],
  [1, "srv-node-1", 26],
].forEach(([u, s, p]: any) => (RACK_A1[14 - u] = { u, server: s, pct: p }));
...
[
  ...
  [4, "srv-node-32", 87],
  [2, "srv-node-2", 27],
].forEach(([u, s, p]: any) => (RACK_A2[14 - u] = { u, server: s, pct: p }));

const RackColumn: React.FC<{
  name: string;
  dc: string;
  temp: string;
  slots: Slot[];
  ...
  return (
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
        <div>
          <div
            style={{ fontFamily: display, fontSize: 17, fontWeight: 600, color: theme.foreground }}
          >
            {name}
          </div>
        ...
        <div
          style={{ fontFamily: mono, fontSize: 13, color: warm ? theme.warning : theme.success }}
        >
          {temp}°C
        </div>
      </div>
      <div style={{ marginTop: 4, fontSize: 11, color: theme.mutedForeground, fontFamily: mono }}>
        Capacity 6 / 42 U · 14%
      </div>
      <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 3 }}>
        {slots.map((slot, i) => {
          ...
          return (
            ...
            >
              <span
                style={{ fontFamily: mono, fontSize: 9, color: theme.mutedForeground, width: 20 }}
              >
                U{14 - i}
              </span>
              {slot ? (
                <>
                  <span style={{ fontFamily: mono, fontSize: 10.5, color: theme.foreground }}>
                    {slot.server}
                  </span>
                  <span
                    style={{
                ...
                <span style={{ fontSize: 9.5, color: theme.mutedForeground, opacity: 0.5 }}>
                  empty
                </span>
              )}
            </div>
          ...
          >
            ...
            {[
              ...
              <div key={r.l} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <span style={{ fontFamily: mono, fontSize: 11, color: theme.foreground }}>
                  {r.v}%
                </span>
              </div>
            ))}
