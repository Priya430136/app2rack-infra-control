...
import {
  Brain, Copy, Download, RefreshCw, Save, AlertTriangle, ShieldAlert,
  Activity, Server as ServerIcon, Cpu, HardDrive, Network as NetIcon,
  ChevronDown, ChevronRight, Clock, Layers, History, Sparkles,
} from "lucide-react";
import { runRcaAnalysis, listRcaAnalyses, deleteRcaAnalysis, type RcaReport } from "@/lib/rca.functions";
...
const riskColor = (r?: string) =>
  r === "Critical" ? "border-destructive/40 bg-destructive/15 text-destructive"
  : r === "High" ? "border-warning/40 bg-warning/15 text-warning"
  : r === "Medium" ? "border-info/40 bg-info/15 text-info"
  : "border-success/40 bg-success/15 text-success";

const confColor = (c: number) =>
  c >= 75 ? "text-success" : c >= 50 ? "text-info" : c >= 30 ? "text-warning" : "text-destructive";

function formatReportText(r: RcaReport, meta: { title: string; severity: string; model: string; source: string }) {
  return [
    ...
    `## Affected Components`, ...r.affected_components.map((c) => `- ${c}`), ``,
    `## Reasoning`, r.reasoning, ``,
    `## Immediate Actions`, ...r.immediate_actions.map((a) => `- ${a}`), ``,
    `## Long-term Recommendations`, ...r.long_term_recommendations.map((a) => `- ${a}`), ``,
    `## Preventive Measures`, ...r.preventive_measures.map((a) => `- ${a}`), ``,
    `Estimated recovery: ${r.estimated_recovery_min} min`,
  ].join("\n");
}

function exportRcaPdf(r: RcaReport, meta: { title: string; severity: string; model: string; source: string }) {
  ...
  for (const [h, body] of sections) {
    ...
    doc.setFont("helvetica", "bold"); doc.setFontSize(11); doc.text(h, 40, y); y += 14;
...
export function RcaPanel({ open, onOpenChange, incident }: Props) {
  ...
  const [current, setCurrent] = useState<{ report: RcaReport; source: string; model: string; savedId: string | null; context: any } | null>(null);
      ...
      >
        <SheetHeader className="sticky top-0 z-10 space-y-1 border-b border-border/60 bg-background/80 p-5 backdrop-blur-xl">
          <div className="flex items-center gap-2">
            ...
            {current && (
              <span className={cn("rounded-md border px-2 py-0.5 text-[10px] uppercase tracking-wider", riskColor(r?.risk_level))}>
                {r?.risk_level} Risk
              </span>
            )}
          </div>
        ...
        <div className="space-y-4 p-5">
          ...
          {runMutation.isError && !current && (
            <div className="rounded-md border border-destructive/40 bg-destructive/10 p-4 text-sm">
              ...
              <p className="text-muted-foreground">{(runMutation.error as any)?.message ?? "The AI service is unavailable."}</p>
          ...
          {r && current && (
            <>
              ...
              <div className="grid grid-cols-3 gap-3">
                <MetaTile label="Confidence" value={`${r.confidence}%`} accent={confColor(r.confidence)} icon={<Sparkles className="h-3.5 w-3.5" />} />
                <MetaTile label="Risk" value={r.risk_level} accent={riskColor(r.risk_level).split(" ")[2]} icon={<ShieldAlert className="h-3.5 w-3.5" />} />
                <MetaTile label="Recovery" value={`${r.estimated_recovery_min}m`} accent="text-info" icon={<Clock className="h-3.5 w-3.5" />} />
              </div>
              ...
              <div>
                ...
                <p className="mt-1 text-[10px] text-muted-foreground">
                  Source: {current.source === "ai" ? "Lovable AI" : "Offline rule engine"} · {current.model}
                </p>
              </div>
              ...
              {/* Root cause */}
              <Section title="Most Probable Root Cause" icon={<AlertTriangle className="h-3.5 w-3.5" />}>
              ...
              <Section title="Infrastructure Affected" icon={<Layers className="h-3.5 w-3.5" />}>
                <div className="flex flex-wrap gap-1.5">
                  {r.affected_components.length === 0 && <span className="text-xs text-muted-foreground">None identified</span>}
                  {r.affected_components.map((c, i) => (
                    <span key={i} className="rounded-md border border-border/60 bg-card/60 px-2 py-0.5 font-mono text-[11px]">{c}</span>
                  ))}
                </div>
              </Section>
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <ListSection title="Immediate Actions" items={r.immediate_actions} tone="destructive" />
                <ListSection title="Long-term Recommendations" items={r.long_term_recommendations} tone="info" />
              </div>
              <ListSection title="Suggested Preventive Measures" items={r.preventive_measures} tone="success" />
              ...
              <div className="sticky bottom-0 -mx-5 flex flex-wrap gap-2 border-t border-border/60 bg-background/90 p-4 backdrop-blur">
                <Button size="sm" variant="outline" onClick={() => { navigator.clipboard.writeText(formatReportText(r, meta)); toast.success("Report copied"); }}>
                  <Copy className="mr-1.5 h-3.5 w-3.5" /> Copy
                </Button>
                <Button size="sm" variant="outline" onClick={() => exportRcaPdf(r, meta)}>
                  <Download className="mr-1.5 h-3.5 w-3.5" /> Export PDF
                </Button>
                <Button size="sm" variant="outline" disabled={!!current.savedId || runMutation.isPending}
                  onClick={async () => { await runMutation.mutateAsync(true); }}>
                  <Save className="mr-1.5 h-3.5 w-3.5" /> {current.savedId ? "Saved" : "Save"}
                </Button>
                <Button size="sm" onClick={() => runMutation.mutate(true)} disabled={runMutation.isPending} className="ml-auto">
                  <RefreshCw className={cn("mr-1.5 h-3.5 w-3.5", runMutation.isPending && "animate-spin")} />
                  Re-run
                </Button>
              </div>
              ...
              <Collapsible open={showHistory} onOpenChange={setShowHistory}>
                ...
                <CollapsibleContent className="mt-2 space-y-2">
                  ...
                  {(history.data ?? []).map((h: any) => (
                    <button
                      key={h.id}
                      onClick={() => setCurrent({ report: h.report, source: h.source, model: h.model, savedId: h.id, context: h.context_snapshot })}
                    ...
                    >
                      <span className={cn("rounded-md border px-2 py-0.5 text-[10px] uppercase", riskColor(h.risk_level))}>
                        {h.risk_level ?? "—"}
                      </span>
                      <span className="font-mono text-muted-foreground">{new Date(h.created_at).toLocaleString()}</span>
                      <span className={cn("ml-auto text-[11px] font-semibold", confColor(h.confidence))}>{h.confidence}%</span>
                      <button
                        onClick={async (e) => { e.stopPropagation(); await del({ data: { id: h.id } }); toast.success("Deleted"); qc.invalidateQueries({ queryKey: ["rca-analyses"] }); }}
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3 rounded-md border border-primary/30 bg-primary/5 p-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-muted-foreground">Collecting metrics, dependencies, history and running AI RCA.</p>
        </div>
      </div>
...
function MetaTile({ label, value, accent, icon }: { label: string; value: string; accent: string; icon: React.ReactNode }) {
  return (
...
function ListSection({ title, items, tone }: { title: string; items: string[]; tone: "destructive" | "info" | "success" }) {
  const toneClass = tone === "destructive" ? "border-destructive/30 bg-destructive/5" : tone === "info" ? "border-info/30 bg-info/5" : "border-success/30 bg-success/5";
  const dot = tone === "destructive" ? "bg-destructive" : tone === "info" ? "bg-info" : "bg-success";
  return (
  ...
  return (
    <div className="grid gap-2 md:grid-cols-2">
      <div className="rounded-md border border-border/60 bg-card/40 p-3 text-xs">
        ...
        <p className="text-muted-foreground">{srv?.os ?? "—"} · {srv?.ip ?? ""}</p>
      ...
      <div className="rounded-md border border-border/60 bg-card/40 p-3 text-xs">
        ...
        <p className="text-muted-foreground">{rack?.dc ?? "—"} · {rack?.temperature_c ?? "?"}°C</p>
        <p className="mt-2 text-muted-foreground">{ctx?.linked_applications?.length ?? 0} linked apps · {ctx?.recent_metrics?.length ?? 0} metric points · {ctx?.similar_incidents?.length ?? 0} similar past incidents</p>
      </div>
      <div className="rounded-md border border-border/60 bg-card/40 p-3 text-xs md:col-span-2">
        ...
        <p className="text-muted-foreground">
          {ctx?.fleet_summary?.total_servers ?? 0} servers · {ctx?.fleet_summary?.critical_count ?? 0} critical · {ctx?.fleet_summary?.offline_count ?? 0} offline
        </p>
      </div>
    </div>
  );
...
function UtilMini({ icon, label, v }: { icon: React.ReactNode; label: string; v?: number }) {
  ...
  return (
    <div>
      ...
      <p className={cn("font-mono text-sm font-semibold", color)}>{typeof v === "number" ? `${v}%` : "—"}</p>
    </div>
  );
}