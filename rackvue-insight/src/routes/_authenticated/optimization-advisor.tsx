...
function ScoreRing({
  ...
  return (
    <Card className="glass-card p-4 flex flex-col items-center gap-2">
      ...
      <span className="text-[11px] text-center text-muted-foreground uppercase tracking-wide">
        {label}
      </span>
    </Card>
  );
  ...
  return (
    <Card className="glass-card p-4 space-y-2">
      <div className="flex items-start justify-between gap-2">
        ...
        <Badge variant="outline" className={`shrink-0 ${PRIORITY_TONE[r.priority] ?? ""}`}>
          {r.priority}
        </Badge>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 pt-1 text-[11px]">
        <div>
          ...
          <span className="font-medium">{r.effort}</span>
        </div>
        <div>
        ...
        <div className="col-span-2">
          ...
          <span className="font-medium">{r.business_impact}</span>
        </div>
        <div className="col-span-2">
          <span className="text-muted-foreground">Gain:</span>{" "}
          <span className="font-medium text-emerald-300">{r.expected_improvement}</span>
        </div>
        {r.target?.name && (
          <div className="col-span-2">
...
function Page() {
  ...
  const runMut = useMutation({
    ...
    onSuccess: (res) => {
      ...
      toast.success(
        res.sourceType === "ai" ? "AI analysis complete" : "Heuristic analysis complete",
      );
    },
            ...
            <div className="flex items-start gap-3 flex-1 min-w-0">
              ...
              <div className="min-w-0">
                <h1 className="text-lg sm:text-2xl font-bold tracking-tight">
                  AI Infrastructure Optimization Advisor
                </h1>
                <p className="text-xs sm:text-sm text-muted-foreground mt-1 leading-relaxed">
                  Continuously analyze applications, servers, racks, health, utilization, incidents
                  and metrics to generate intelligent optimization recommendations.
                </p>
              </div>
            </div>
            <Button
          ...
          <Card className="glass-card p-8 sm:p-12 text-center">
            <div className="max-w-md mx-auto space-y-3">
              ...
              <p className="text-sm text-muted-foreground">
                Click{" "}
                <span className="font-medium text-foreground">Run Infrastructure Analysis</span> to
                have the AI review your entire environment and surface actionable recommendations.
              </p>
            </div>
          </Card>
        )}
        ...
        {report && (
          <>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-9 gap-3">
              ...
              <ScoreRing
                label="Cooling"
                value={scores!.cooling_efficiency}
                icon={Thermometer}
                tone="warn"
              />
            </div>
                  ...
                  onClick={() =>
                    exportPDF({
                      ...
                      rows: [
                        ...
                        {
                          Metric: "Est. monthly savings ($)",
                          Score: report.cost_savings.monthly_estimate,
                        },
                      ],
                    })
                  }
              ...
              <Card className="glass-card p-4 lg:col-span-1">
                ...
                <div className="h-56">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        data={report.cost_savings.breakdown}
                        dataKey="amount"
                        nameKey="area"
                        outerRadius={70}
                        label={{ fontSize: 10 }}
                      >
                        {report.cost_savings.breakdown.map((_, i) => (
                ...
                <div className="text-center mt-2">
                  <div className="text-2xl font-bold text-emerald-400">
                    ${report.cost_savings.monthly_estimate.toLocaleString()}/mo
                  </div>
                  <div className="text-[11px] text-muted-foreground">
                    ${report.cost_savings.yearly_estimate.toLocaleString()} / year potential savings
                  </div>
                </div>
              </Card>
            </div>
                      ...
                      <div className="flex-1 h-6 bg-muted/30 rounded overflow-hidden relative">
                        <div
                          ...
                          style={{
                            width: `${r.risk}%`,
                            background:
                              r.risk > 70 ? "#ef4444" : r.risk > 50 ? "#f59e0b" : "#10b981",
                          }}
                        />
                        <span className="absolute inset-0 flex items-center justify-end pr-2 text-[10px] text-white/90">
                          {r.risk}%
                        </span>
                      </div>
                    ...
                    {report.server_matrix.map((s) => (
                      <div key={s.server} className="grid grid-cols-4 gap-2 items-center text-xs">
                        ...
                        <div>
                          ...
                          <Progress value={s.ram} className="h-1.5" />
                        </div>
                        <div>
                          <div className="text-[10px] text-muted-foreground">Disk</div>
                          <Progress value={s.storage} className="h-1.5" />
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <Tabs defaultValue="performance" className="w-full">
                <div className="overflow-x-auto -mx-4 px-4 sm:mx-0 sm:px-0">
                  <TabsList className="w-max">
                    ...
                    <TabsTrigger value="rack" className="text-xs">
                      Rack
                    </TabsTrigger>
                    ...
                    <TabsTrigger value="incident_prevention" className="text-xs">
                      Incident Prev
                    </TabsTrigger>
                    ...
                    <TabsTrigger value="high_availability" className="text-xs">
                      HA
                    </TabsTrigger>
                  </TabsList>
                </div>
                {(Object.keys(report.categories) as Array<keyof typeof report.categories>).map(
                  (k) => (
            ...
            <Card className="glass-card p-4 sm:p-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                {[
                  {
                    label: "Immediate",
                    items: report.timeline.immediate,
                    tone: "border-red-500/30 bg-red-500/5",
                  },
                  {
                    label: "This Week",
                    items: report.timeline.this_week,
                    tone: "border-amber-500/30 bg-amber-500/5",
                  },
                  {
                    label: "This Month",
                    items: report.timeline.this_month,
                    tone: "border-cyan-500/30 bg-cyan-500/5",
                  },
                  {
                    label: "Long-Term",
                    items: report.timeline.long_term,
            ...
            <Card className="glass-card p-4 sm:p-5">
                ...
                <ScrollArea className="h-56 mb-3 border border-border/40 rounded-lg p-3 bg-background/30">
                  <div className="space-y-3">
                    {chat.map((m, i) => (
                      <div
                        key={i}
                        className={`text-sm ${m.role === "user" ? "text-cyan-300" : "text-foreground"}`}
                      >
              ...
              >
                <Input
                  ...
                  placeholder="Ask about infrastructure cost, upgrades, availability…"
          ...
          {history.isLoading ? (
            ...
            <div className="space-y-2">
              {(history.data ?? []).map((h: any) => (
                <div
                  key={h.id}
                  className="flex items-center gap-3 rounded-lg border border-border/40 bg-background/30 p-3 hover:bg-background/50 transition"
                >
                  ...
                  <div className="hidden sm:flex items-center gap-3 text-[11px]">
                    ...
                    <Badge variant="outline" className="text-emerald-300 border-emerald-500/30">
                      ${Number(h.estimated_savings_monthly ?? 0).toLocaleString()}/mo
                    </Badge>
                  </div>
                  <Button size="sm" variant="ghost" onClick={() => setReport(h.report)}>
                    View
                  </Button>
