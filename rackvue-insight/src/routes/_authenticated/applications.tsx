        ...
        <Card className="border-border/60 bg-card/60 p-4 backdrop-blur">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-1 rounded-md border border-border/60 bg-background/40 p-1">
              {["All", "Production", "UAT", "Dev"].map((e) => (
                <button
                  key={e}
                  onClick={() => setEnv(e)}
                  className={`rounded px-3 py-1 text-xs font-medium transition ${env === e ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}
                >
                  {e}
                </button>
              ))}
            ...
            <ExportMenu
              ...
              subtitle="Application inventory"
            />
                ...
                <tbody>
                  {filtered.map((a) => {
                    ...
                    return (
                      <tr
                        key={a.id}
                        className="border-b border-border/30 last:border-0 hover:bg-background/30"
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1">
                            ...
                            <Button
                              ...
                              variant="ghost"
                              className="h-7 w-7"
                              onClick={() => dm.mutate({ id: a.id, archive: true })}
                            >
                            ...
                            <Button
                              ...
                              onClick={() => {
                                if (confirm("Delete this application?")) dm.mutate({ id: a.id });
                              }}
                            >
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="grid gap-3">
            ...
            <div className="grid gap-1.5">
              <Label>Server</Label>
              <Select
                value={form.server_id ?? "__none"}
                onValueChange={(v) => setForm({ ...form, server_id: v === "__none" ? null : v })}
              >
                <SelectTrigger>
