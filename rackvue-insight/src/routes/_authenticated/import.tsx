...
const CANONICAL: Record<Target, string[]> = {
  servers: ["name", "hostname", "ip", "os", "cpu_usage", "memory_usage", "storage_tb", "status", "rack_id", "slot"],
...
function Page() {
  ...
  const importMut = useMutation({
    mutationFn: () => doImport({ data: { filename: file!.name, kind, target, rows: rows as any, mapping } }),
    onSuccess: (res: any) => {
      qc.invalidateQueries();
      toast.success(`Imported ${res.imported} rows${res.errors?.length ? ` (${res.errors.length} errors)` : ""}`);
      setFile(null); setRows([]); setHeaders([]); setMapping({});
    },
  ...
  const delMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["datasets"] }); toast.success("Deleted"); },
  });

  const handleFile = useCallback(async (f: File) => {
    setFile(f);
    const ext = f.name.toLowerCase().split(".").pop();
    const k: "csv" | "xlsx" | "json" = ext === "json" ? "json" : ext === "xlsx" || ext === "xls" ? "xlsx" : "csv";
    setKind(k);
    let parsed: Array<Record<string, unknown>> = [];
    if (k === "csv") {
      const text = await f.text();
      const r = Papa.parse<Record<string, unknown>>(text, { header: true, skipEmptyLines: true, dynamicTyping: true });
      parsed = r.data;
    } else if (k === "json") {
      const text = await f.text();
  ...
  return (
    <>
      <TopBar title="Data Import Center" subtitle="Upload CSV, Excel, or JSON to populate your infrastructure" />
      <div className="space-y-6 p-6">
        <Card className="border-border/60 bg-card/60 p-6 backdrop-blur">
          <div className="grid gap-4 md:grid-cols-[1fr_220px]">
            ...
            >
              ...
              <p className="mt-1 text-xs text-muted-foreground">Supports CSV, XLSX, JSON · up to 5,000 rows</p>
              <input type="file" accept=".csv,.xlsx,.xls,.json" className="absolute inset-0 cursor-pointer opacity-0"
                onChange={(e) => { const f = e.target.files?.[0]; if (f) handleFile(f); }} />
              {file && <p className="mt-3 text-xs text-primary">{file.name} · {rows.length} rows</p>}
            </div>
            <div className="grid gap-3">
              ...
              <Button disabled={!rows.length || importMut.isPending} onClick={() => importMut.mutate()}>
                ...
                <tbody>
                  {rows.slice(0, 10).map((r, i) => (
                    <tr key={i} className="border-t border-border/30">
                      {headers.map((h) => <td key={h} className="px-3 py-1.5 font-mono">{String(r[h] ?? "")}</td>)}
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <Card className="border-border/60 bg-card/60 p-6 backdrop-blur">
          ...
          {datasets.isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> :
            !datasets.data?.length ? <p className="text-xs text-muted-foreground">No imports yet.</p> :
            <div className="space-y-2">
              {datasets.data.map((d) => (
                <div key={d.id} className="flex items-center gap-3 rounded-md border border-border/40 bg-background/30 px-3 py-2 text-xs">
                  <FileText className="h-4 w-4 text-muted-foreground" />
                  <span className="font-medium">{d.filename}</span>
                  <span className="rounded bg-muted/40 px-1.5 py-0.5 uppercase tracking-wider">{d.kind}</span>
                  <span className="text-muted-foreground">→ {d.target_entity}</span>
                  <span className="ml-auto flex items-center gap-1">
                    {d.status === "imported" ? <CheckCircle2 className="h-3.5 w-3.5 text-success" /> : <AlertCircle className="h-3.5 w-3.5 text-destructive" />}
