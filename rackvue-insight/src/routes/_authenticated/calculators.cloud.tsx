...
const PRICING = {
  aws: { instance: 60, ramGB: 4.2, storageGB: 0.1, bandwidthGB: 0.09, backupGB: 0.05 },
...
function CloudCalculator() {
  ...
  const historyQ = useQuery({
    queryKey: ["calc-history", "cloud"],
    queryFn: () => listFn({ data: { kind: "cloud" } }),
  });
  const history = (historyQ.data ?? []) as Array<{
    id: string;
    inputs: Inputs;
    outputs: { aws: number; azure: number; gcp: number };
  }>;
  ...
  const result = useMemo(() => {
    ...
    const tips: string[] = [];
    if (inp.bandwidthGB > 1000)
      tips.push("Bandwidth is a major cost driver — use CDN caching to reduce egress by 40–60%.");
    if (inp.backupGB > inp.storageGB)
      tips.push("Backup exceeds primary — move cold backups to archival tier (Glacier/Archive).");
    if (inp.instances > 5)
      tips.push(
        "Consider Reserved Instances or Savings Plans for 30–60% savings on steady workloads.",
      );
    if (inp.ramGB / Math.max(1, inp.instances) > 32)
      tips.push("High RAM per instance — review for memory-bound workload rightsizing.");
    tips.push(
      `Cheapest provider for this profile: ${cheapest[0]} (~$${Math.round(cheapest[1] as number).toLocaleString()}/mo).`,
    );
    return {
      data: [
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["calc-history", "cloud"] });
  ...
  return (
    <>
      <TopBar
        ...
        subtitle="Compare AWS, Azure & GCP for your workload profile"
      />
        ...
        <Card className="p-5">
          ...
          <div className="grid gap-3">
            <NumField
              ...
              value={inp.instances}
              onChange={(v) => setInp({ ...inp, instances: v })}
            />
            <NumField
              icon={<MemoryStick className="h-3.5 w-3.5" />}
              label="Total RAM (GB)"
              value={inp.ramGB}
              onChange={(v) => setInp({ ...inp, ramGB: v })}
            />
            <NumField
              icon={<HardDrive className="h-3.5 w-3.5" />}
              label="Storage (GB)"
              value={inp.storageGB}
              onChange={(v) => setInp({ ...inp, storageGB: v })}
            />
            <NumField
              label="Monthly Bandwidth (GB)"
              value={inp.bandwidthGB}
              onChange={(v) => setInp({ ...inp, bandwidthGB: v })}
            />
            <NumField
              label="Backup Storage (GB)"
              value={inp.backupGB}
              onChange={(v) => setInp({ ...inp, backupGB: v })}
            />
            <div className="grid gap-1.5">
              ...
              <Select
                value={inp.region}
                onValueChange={(v) => setInp({ ...inp, region: v as Inputs["region"] })}
              >
          ...
          <Card className="p-5">
            ...
            <ul className="grid gap-2 text-xs text-muted-foreground">
              {result.tips.map((t, i) => (
                <li
                  key={i}
                  className="flex gap-2 rounded-md border border-border/40 bg-muted/20 p-2.5"
                >
                ...
                {history.map((h) => (
                  ...
                  >
                    ...
                    <span className="mx-3 font-mono">
                      AWS ${h.outputs.aws.toLocaleString()} · Azure $
                      {h.outputs.azure.toLocaleString()} · GCP ${h.outputs.gcp.toLocaleString()}
                    </span>
                    <button
