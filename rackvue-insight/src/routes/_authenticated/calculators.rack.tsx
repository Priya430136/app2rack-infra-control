...
function RackCalculator() {
  ...
  const historyQ = useQuery({
    queryKey: ["calc-history", "rack"],
    queryFn: () => listFn({ data: { kind: "rack" } }),
  });
  const history = (historyQ.data ?? []) as Array<{
    id: string;
    inputs: Inputs;
    outputs: { racks: number; totalPowerKW: number };
  }>;
    ...
    mutationFn: () =>
      saveFn({
        data: {
          kind: "rack",
          inputs: inp,
          outputs: { racks: calc.racks, totalPowerKW: calc.totalPowerKW },
        },
      }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["calc-history", "rack"] });
  ...
  return (
    <>
      <TopBar
        title="Rack Capacity Calculator"
        subtitle="Plan rack units, power & cooling for your fleet"
      />
        ...
        <Card className="p-5">
          ...
          <div className="grid gap-3">
            <NumField
              ...
              value={inp.servers}
              onChange={(v) => setInp({ ...inp, servers: v })}
            />
            <NumField
              label="Server Size (U)"
              value={inp.uPerServer}
              onChange={(v) => setInp({ ...inp, uPerServer: v })}
            />
            <NumField
              label="Power per Server (W)"
              value={inp.powerW}
              onChange={(v) => setInp({ ...inp, powerW: v })}
            />
            <NumField
              label="Cooling Factor"
              value={inp.coolingFactor}
              onChange={(v) => setInp({ ...inp, coolingFactor: v })}
              step={0.1}
            />
            <NumField
              label="Redundancy Factor"
              value={inp.redundancy}
              onChange={(v) => setInp({ ...inp, redundancy: v })}
              step={0.1}
            />
          </div>
          <div className="mt-4">
        ...
        <div className="grid gap-4">
          <div className="grid gap-4 md:grid-cols-4">
            ...
            <Metric
              ...
              value={String(calc.racks)}
            />
            <Metric
              icon={<Zap className="h-4 w-4" />}
              label="Power"
              value={`${calc.totalPowerKW.toFixed(1)} kW`}
            />
            <Metric
              icon={<Snowflake className="h-4 w-4" />}
              label="Cooling"
              value={`${calc.coolingTons.toFixed(1)} tons`}
            />
          </div>
          ...
          <Card className="p-5">
            <div className="mb-3 flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground">
                Space utilization:{" "}
                <span className="font-mono text-foreground">{calc.utilization}%</span>
              </span>
            </div>
                ...
                {history.map((h) => (
                  ...
                  >
                    ...
                    <span className="mx-3 font-mono">
                      {h.outputs.racks} racks · {h.outputs.totalPowerKW.toFixed(1)} kW
                    </span>
                    <button
